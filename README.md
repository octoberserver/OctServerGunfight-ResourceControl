...
#### Q: 刪除 Loot Table 會影響已應用它的箱子嗎？
**A:** 不會。刪除 Loot Table 後，箱子會保留對它的引用，但無法再生成物品。如果需要恢復，請重新創建相同名稱的 Loot Table。
...
- `CustomLootTable.getItems()` - 獲取所有物品
- `CustomLootTable.getTotalWeight()` - 獲取總權重
...
- `CustomLootTableManager.deleteCustomLootTable(String name)` - 刪除 Loot Table
- `CustomLootTableManager.hasLootTable(String name)` - 檢查 Loot Table 是否存在
...
### v1.6.0 (最新)
- ✅ **新增功能：自定義 Loot Table**
- ✅ 新增 `/customloot create <名稱> [權重...]` 指令
- ✅ 新增 `/customloot list` 指令
- ✅ 新增 `/customloot info <名稱>` 指令
- ✅ 新增 `/customloot delete <名稱>` 指令
- ✅ 新增 `/customloot apply <箱子> <表>` 指令
- ✅ 支持从背包的 27 格物品创建 Loot Table
- ✅ 支持为每个物品设置权重
- ✅ 新增 `CustomLootTable.java` 类
- ✅ 新增 `CustomLootTableManager.java` 类
- ✅ 修改 `ChestManager.clearAndApplyLootTable` 支持自定义 Loot Table
- ✅ 修改 `ChestManagerHolder` 支持自定义 Loot Table 管理器
...
# Loot Table 名稱
dungeon_common, boss_treasure, event_halloween
```
...
- 使用有意義的名稱
- 定期使用 `/customloot list` 和 `/chest list` 檢查配置
- 使用備份來保存重要的配置
- 在大型項目中使用組來組織箱子
- 測試不同的種子以確保物品分佈符合預期
...
- 不要使用超過 27 個不同的物品在自定義 Loot Table 中
- 不要忘記備份重要的數據
